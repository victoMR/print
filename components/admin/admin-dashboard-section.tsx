    ...
    <BotySurface className="p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="font-serif text-2xl sm:text-3xl mt-1 leading-tight">{value}</p>
  ...
  if (series.length === 0) {
    return (
      <div className="text-center py-14 px-4">
        ...
        <p className="text-sm font-medium text-foreground">Aún no hay ventas en estas fechas</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-4 text-sm">
        <p>
          <span className="text-muted-foreground">Total en el gráfico: </span>
          ...
          return (
            ...
            >
              {showValuesOnBars && hasSales && (
                <span className="text-[10px] sm:text-xs font-medium text-foreground text-center leading-tight px-0.5">
                  {formatMxn(row.revenueMxn)}
                </span>
              )}
              <div
                className={cn(
                  "w-full max-w-[52px] rounded-t-xl boty-transition",
                  hasSales ? "bg-primary hover:bg-primary/90" : "bg-muted/80",
                )}
                style={{ height: `${hasSales ? Math.max(pct, 8) : 4}%` }}
                title={`${row.label}: ${formatMxn(row.revenueMxn)} · ${row.orders} pedido${row.orders !== 1 ? "s" : ""}`}
              />
              <span className="text-[10px] sm:text-xs text-muted-foreground text-center leading-tight w-full truncate px-0.5">
                {row.label}
              </span>
              {row.orders > 0 && (
...
export function AdminDashboardSection({ onError, refreshKey = 0 }: AdminDashboardSectionProps) {
  ...
  return (
    <section className="space-y-6">
      ...
      <BotySurface className="p-5 space-y-4">
        <div className="flex items-start gap-3">
          <CalendarDays className="w-5 h-5 text-primary shrink-0 mt-0.5" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium">¿Qué fechas quieres ver?</p>
            <p className="text-xs text-muted-foreground mt-0.5">{selectedPeriodHint}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {PERIOD_OPTIONS.map((opt) => (
            <button
              ...
              className={cn(
                "px-4 py-2.5 rounded-2xl text-sm font-medium boty-transition border",
                period === opt.value
        ...
        {period === "custom" && (
          <div className="flex flex-wrap gap-4 items-end pt-1 border-t border-border/40">
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center gap-2 text-muted-foreground text-sm py-20">
      ...
      {!loading && period === "custom" && (!customFrom || !customTo) && (
        <BotySurface className="p-10 text-center">
          <CalendarDays className="w-10 h-10 text-muted-foreground/50 mx-auto mb-3" />
          <p className="font-medium">Elige un rango de fechas</p>
      ...
      {!loading && data && (
        <>
          <BotySurface className="p-6 sm:p-8 bg-gradient-to-br from-primary/5 via-card to-card border-primary/10">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="font-serif text-3xl sm:text-4xl mt-3 text-foreground">
                  {formatMxn(data.summary.revenueMxn)}
                </p>
                <p className="text-sm text-muted-foreground mt-2">
                  {hasSales ? (
                    <>
                      <span className="font-medium text-foreground">{data.summary.paidOrders}</span>
                      {data.summary.paidOrders === 1 ? " pedido pagado" : " pedidos pagados"}
                      {data.summary.itemsSold > 0 && (
                        <> · <span className="font-medium text-foreground">{data.summary.itemsSold}</span> artículos</>
                      )}
                    </>
                  ) : (
              ...
              {hasSales && (
                <div className="rounded-2xl bg-background/80 border border-border/50 px-4 py-3 text-sm">
                  <p className="text-muted-foreground text-xs">Promedio por pedido</p>
                  <p className="font-semibold text-lg mt-0.5">{formatMxn(data.summary.avgOrderMxn)}</p>
                </div>
              )}
            </div>
          </BotySurface>
          ...
          {data.summary.cancelledOrders > 0 && (
            <BotySurface className="px-4 py-3 flex items-center gap-3 text-sm border-amber-500/20 bg-amber-500/5">
              ...
              <p>
                ...
                (pueden incluir abandonos o cancelaciones del equipo).
              </p>
            </BotySurface>
          )}

          <div className="grid gap-6 lg:grid-cols-3">
            <BotySurface className="p-5 sm:p-6 lg:col-span-2">
              <h2 className="font-serif text-xl">¿Cómo fueron las ventas día a día?</h2>
            ...
            <BotySurface className="p-5 sm:p-6">
              <h2 className="font-serif text-xl">Estado de los pedidos</h2>
              <p className="text-sm text-muted-foreground mt-1 mb-5">
                En qué paso del envío están los pedidos pagados.
              </p>
              {data.byStatus.length === 0 ? (
                <p className="text-sm text-muted-foreground">No hay pedidos pagados en estas fechas.</p>
              ) : (
                    ...
                    return (
                      <li key={row.status}>
                        ...
                        <div className="h-2.5 rounded-full bg-muted overflow-hidden">
                          <div
                            className="h-full bg-primary rounded-full boty-transition"
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                      </li>
          ...
          <BotySurface className="p-5 sm:p-6 overflow-hidden">
            <h2 className="font-serif text-xl">Productos más vendidos</h2>
            <p className="text-sm text-muted-foreground mt-1 mb-5">
              Los que más dinero generaron en el periodo seleccionado.
            </p>
            {data.topProducts.length === 0 ? (
              <div className="text-center py-10">
                <Package className="w-9 h-9 text-muted-foreground/40 mx-auto mb-2" />
                <p className="text-sm text-muted-foreground">Todavía no hay productos vendidos aquí.</p>
              </div>
            ) : (
                ...
                {data.topProducts.map((row, index) => (
                  ...
                  >
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-semibold">{formatMxn(row.revenueMxn)}</p>
