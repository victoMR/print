import { languageForLocale, type Locale } from "./locale";
...
export function localizedProductName(
  ...
  if (languageForLocale(locale) === "en" && product.nameEn) return product.nameEn;
  return product.name;
}
...
export function localizedProductDescription(
  ...
  if (languageForLocale(locale) === "en" && product.descriptionEn) return product.descriptionEn;
